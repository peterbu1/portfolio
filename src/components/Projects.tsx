...
const Projects: React.FC<ProjectsProps> = ({ projects }) => {
  return (
    <motion.div
      ...
      <div className="w-full flex space-x-5 overflow-x-scroll p-10 snap-x snap-mandatory scrollbar scrollbar-track-gray-400/20 scrollbar-thumb-[#005a7a]/80">
        ...
        {projects.map((project) => (
          ...
          {/* bg-[#292929] flex flex-col flex-shrink-0 w-[300px] md:w-[400px] xl:w-[500px] xl:h-[500px] snap-center p-4 shadow-md rounded-md transition-transform hover:scale-105 */}
            ...
            <p className="text-zinc-400 mb-4 desc">{project.description}</p>
            <h4 className="text-sm font-medium mb-2">Technologies used:</h4>
            <ul className="mb-4">
              {project.technologies.map((tech) => (
                <li key={tech} className="inline-block bg-cyan-300 text-gray-800 px-2 py-1 rounded-md text-sm mr-2 mb-2">
                  {tech}
                </li>
              ))}
            </ul>
            <div className="flex space-x-2">
              {project.links.website && (
                <a href={project.links.website} target="_blank" rel="noopener noreferrer" className="bg-emerald-500 hover:bg-emerald-600 text-white py-2 px-4 rounded-md text-sm">
                  Visit Website
                </a>
              )}
...
const ExampleComponent: React.FC = () => {
  const projects: Project[] = [
    ...
    {
      id: 3,
      name: 'Whats Your Commute?',
      photo: '/commute.png',
      description: `\"What's the Commute?\" is a user-friendly web application designed to assist individuals in finding the ideal homes and areas that strike a balance in their commute. By leveraging the power of the Google Maps API, it accurately calculates the distance and duration between locations, providing valuable insights into travel time, costs, and environmental impact. Users can input their car's fuel efficiency, gas prices, and weekly commutes to estimate annual gas costs and carbon footprints. With an intuitive interface, and customizable parameters \"What's the Commute?\" empowers users to make informed decisions about their work-life balance and transportation choices while optimizing their commuting experience.`,
      technologies: ['HTML', 'React', 'JavaScript', 'TypeScript', 'Google Maps API'],
      links: {
        website: 'https://whats-the-commute.vercel.app',
        github: 'https://github.com/peterbu1/Whats-the-Commute',
      }
    },
    {
      id: 4,
  ...
  return (
    <div
      ...
      <h1 className="absolute top-0 md:top-14 uppercase tracking-[20px] text-gray-500 text-2xl">My Projects</h1>
